...
priors2 <- c(
  prior(horseshoe(df = 1, scale_global = tau0), class = "b"),
  prior(normal(0,1), class = "Intercept"),
...
m0 <- 10    # change to 3 or 10 to test sensitivity
...
```{r}
brms_model2 <- brms::brm(as.numeric(carrier) ~ . - SITEID + (1 | SITEID),
                        dataset, family = brms::bernoulli(link = "logit"),
                        prior = priors3,
                        chains = 4,
                        cores = parallel::detectCores(),
...
summary(brms_model2) -> model10 #expected number of non-zero coefficients == 10
```
...
#bad_rhats <- rhat(brms_model2)[rhat(brms_model2) > 1.01]
...
sampler_diagnostics <- nuts_params(brms_model2)

# trace plots for a few parameters
mcmc_trace(as_draws_df(brms_model2)[, c(intersect(names(as_draws_df(brms_model2)), c("age","sd_SITEID__Intercept")))])
```

## Model checks

```{r}
pp_check(brms_model2, ndraws = 200)   # posterior predictive check

loo_model <- loo(brms_model2, moment_match = FALSE)
print(loo_model)

print(bayes_R2(brms_model2))
```

## Posterior summaries

```{r}
post_summary <- posterior_summary(brms_model2)
post_summary[grep("^b_", rownames(post_summary)), , drop = FALSE]  # show coefficients

# Posterior draws
draws <- as_draws_df(brms_model2)
