...
survey[["DEMO_HIS"]] <- survey[["DEMO_HIS"]] %>%
  mutate(JOB_final = case_when(JOB_SPECIFY %in% c("Cán bộ công chức", "Cán bộ xã") ~ 1,
                               JOB_SPECIFY %in% c("Bảo vệ", "Cơ khí","Hộ lý",
                                                  "Lái tàu thủy", "Lao động tự do",
                                                  "Vệ sỹ") ~ 4,
                               JOB_SPECIFY %in% c("già", "Già",
                                                  "Hết độ tuổi lao động") ~ 8,
         ...
         smoking = as.factor(ifelse(SMOKING == 1, "current smoker",
                                    ifelse(SMOKING == 2, "former smoker",
                                           "all non-smokers"))),
         ...
         DrugCAGE = DRUG_CUTDOWN_score + DRUG_ANNOYED_score + DRUG_GUILTY_score +
           DRUG_DRUG_score,
         job = as.factor(ifelse(JOB_final== 6, "farmer",
                                ifelse(JOB_final == 7, "housewife",
                                       ifelse(JOB_final == 8, "retired",
                                              "all other jobs")))),
