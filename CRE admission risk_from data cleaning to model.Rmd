...
```{r}
required <- c("dplyr", "purrr", "tidyr", "stringr", "magrittr", "readxl", "gmodels",
              "lsmeans", "lme4", "glmmLasso", "rlang")
```
...
```{r}
survey <- c("SCR", "ADM", "DEMO_HIS", "DEMO_HIS_HISTORY_GRID", "DIS", "ANT",
            "ANT_ANB_HOSP_GRID") |> 
  set_names() |> 
  map(read_excel2, file = CRF_file) |>
  map(dplyr::filter, entry == 2)
```
...
The `SCR` tab patched with the dates of discharge of the patients who did not provide
any sample at discharge:
...
The `DEMO_HIS_HISTORY_GRID` tab:

```{r}
survey$DEMO_HIS_HISTORY_GRID <- survey$DEMO_HIS_HISTORY_GRID |>
...
```{r}
survey$ANT <- survey$ANT |>
  select(USUBJID, ANB_HOS) |> 
  mutate(across(ANB_HOS, ~ .x > 2))
```

The `ANT_ANB_HOSP_GRID` tab:

```{r}
survey$ANT_ANB_HOSP_GRID <- survey$ANT_ANB_HOSP_GRID |>
  ...
  mutate(agent = case_when(ANB_OTHER_SPEC %in% cefoperazone  ~ "Cefoperazone",
                           ...
                           ANB_OTHER_SPEC %in% cefo_sulb     ~ "Cefoperazon_sulbactam",
...
Calculating DOT of antibiotics used in hospital for each patient:

```{r}
survey$antibiotic_hospital_DOT <- survey$ANT_ANB_HOSP_GRID %>%
  mutate(number = 1) |> 
  group_by(USUBJID) %>%
  summarise(number = sum(number), total_DOT = sum(DOT)) |> 
  select(-number)
```
