...
# factor / character
  if (is.factor(x) || is.character(x)) {
    data |> 
      ...
      mutate(across(variable, ~ paste0(var, .x)),
             N   = sum(n),
             pct = n / N) |> 
      return()
  }
...
```{r}
f <- function(var = "estimate_quartile") {
data |> 
  group_by(.data[[var]]) |> 
  tally() |> 
  rename_with(~ "variable", .cols = 1) |> 
