...
if (Sys.getenv("USER") == "MarcChoisy") {
  path2data <- paste0("~/Library/CloudStorage/",
                      "OneDrive-OxfordUniversityClinicalResearchUnit/SharePoint/",
                      "AMS Data Analysis and Modelling Team/CRE gut acquisition/data/")
  #path2cache <- 
} else {
...
~/Library/CloudStorage/OneDrive-OxfordUniversityClinicalResearchUnit/SharePoint/AMS Data Analysis and Modelling Team/CRE gut acquisition/data
...
abx_early <- CRF$ANT_ANB_HOSP_GRID |>
  ...
  group_by(USUBJID) |>
  summarise(admission_date   = first(as.Date(ADMISSION_DATE)),
            early_any_abx    = n() > 0,
            early_DOT        = n(),
            early_carb_DOT   = sum(carbapenem),
            first_abx_day    = min(day),
            .groups = "drop") |>
  mutate(days_to_first_abx = as.integer(first_abx_day - admission_date)) |>
...
vars <- c("estimate","upper","lower","estimate_quartile", "otherCRE", "age", "GENDER",
