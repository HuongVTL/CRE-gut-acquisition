...
It seems the output function from the `binom.test()` above could have issues, use the following instead:

```{r}
safe_binom_test <- function(x, n) {
...
To fix the issues with binom test, use the following:
...
proportion_CRE <- ward_dates |>
  group_by(ward) |>
  arrange(.data$ADMISSION, .by_group = TRUE) |>
  group_modify(~ ward_occupancy(.x)) |>
  ungroup() |>
  rename(denominator = occupancy) |>
  left_join(numerator_df, by = c("ward", "date")) |>
  mutate(
    numerator = replace_na(numerator, 0L),
    denominator = as.integer(round(denominator)),
    numerator = as.integer(round(numerator))) |>
  mutate(
  bt = map2(
      numerator, denominator,
      ~ if (!is.na(.x) && !is.na(.y) && .y > 0 && .x >= 0 && .x <= .y) binom.test(.x, .y) else NULL)) |>
  mutate(
    estimate = map_dbl(bt, ~ if (is.null(.x)) NA_real_ else unname(.x$estimate)),
    lower = map_dbl(bt, ~ if (is.null(.x)) NA_real_ else .x$conf.int[1]),
    upper = map_dbl(bt, ~ if (is.null(.x)) NA_real_ else .x$conf.int[2])) |>
  select(-bt)
```
...
### **9.2 Multi-state modelling**

#### **9.2.1 State definition and transition structure**
...
#### **10.2.1 State definition and transition structure**
