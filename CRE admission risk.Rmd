---
title: "CRE admission risk model"
...
##Preparing clean_data for Lasso
...
#Generate initial fixed and random estimates and co-variance

```{r fixed and mixed models}
...
```{r find best lambda for glmmLasso, warning=FALSE}
...
for(j in 1:length(lambda_seq)){
  ...
  glmm_temp <- try(glmmLasso(
    ...
    switch.NR = TRUE,
    final.re = FALSE,
    control=list(start=Delta.start,q_start=Q.start), print.iter = FALSE, print.iter.final = FALSE),
    silent = TRUE)
...
round(cbind(exp(tab),p=coef(summary(CRE_admission_model))[,4]),digits = 3) 
