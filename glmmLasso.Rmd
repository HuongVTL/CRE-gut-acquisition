...
vars_to_keep <- c(
  "CRE_carriage", "age_s", "gender", "smoking2", "hospital", "children2", 
  "married", "ambulatory", "job_farmer", "job_housewife", "job_retired", 
  "cormobidity", "heart_diseases", "vascular_diseases", "dementia", "cpd", "ctd","pud",
  "mild_liver", "sev_liver", "hemiplegia", "sev_renal", "diabetes_noeod",
  "diabetes_eod", "cancer",
  "alcohol", "drug","wardtype", "InvasiveHistory", "MedicationHistory","HealthcareHistory", "hospitaltype", 
  "ICD_Circulatory", "ICD_Genitourinary", "ICD_IDIntestinal", "ICD_IDOther", "ICD_Neoplasms", "ICD_Respiratory", "ICD_Digestive")
...
mixed_formula <- CRE_carriage ~ age_s + gender+smoking2+children2+married+ambulatory+job_farmer+job_housewife+job_retired +cormobidity+vascular_diseases+cpd+ctd+pud+mild_liver+sev_liver+hemiplegia+sev_renal+diabetes_noeod+diabetes_eod+ cancer+alcohol +drug+wardtype+InvasiveHistory +MedicationHistory+HealthcareHistory + hospitaltype+ICD_Circulatory + ICD_Genitourinary + ICD_IDIntestinal + ICD_IDOther + ICD_Neoplasms+ICD_Respiratory+ICD_Digestive+
                            (1 | hospital)  #not include heart_diseases and dementia yet -

simple_mixed_model <- glmer(mixed_formula, data = riskdata_scaled, family = binomial(link = "logit"))
initial_fixed_estimates <- fixef(simple_mixed_model)
initial_random_estimates <- ranef(simple_mixed_model)
VarCorr(simple_mixed_model)

Delta.start<-c(as.numeric(initial_fixed_estimates),rep(0,6),
...
```{r Fit Model using glmmLasso}
## Fit the Model using glmmLasso: 
## find best lambda

# Define a sequence of lambda values (from high penalty to low)
lambda_seq <- seq(50, 0, by = -1)
...
CRE_admission_model <- glmmLasso(
  ...
  rnd=list(hospital = ~1),
  data = data_clean,
  lambda = 10, # larger -> stronger regularization and more coefficients shrunk to zero; smaller reduces penalty
