...
get_nN_1 <- function(x) {
  column <- unlist(x)
  if (! (is.logical(column) | is.factor(column) | is.character(column))) return(NULL)
  x |> 
    group_by(across(1)) |> 
    tally() |> 
    rename_with(~ "variable", .cols = 1) |> 
    mutate(across(variable, ~ paste0(names(x), "_", .x)),
           N   = sum(n),
           pct = n / N)
}
...
```{r ward_occupancies, fig.height = 10.38, fig.width  = 8.3, purl = FALSE, eval = eval_big_fig}
...
opar <- par(mfrow = c(4, 2), mar = c(2, 2, 1, 1)) #my window cannot view the mfrow = c(8, 2)
ward_dates |> 
  group_by(ward) |> 
...
```{r enrolled_CRE_patients, fig.height = 10.38, fig.width  = 8.3, purl = FALSE, eval = eval_big_fig}
...
```{r prop_patients_with_CRE, fig.height = 10.38, fig.width  = 8.3, purl = FALSE, eval = eval_big_fig}
opar <- par(mfrow = c(4, 2), mar = c(2, 2, 1, 1))
proportion_CRE |> 
  group_by(ward) |> 
...
```{r enrolled_Kp_patients, fig.height = 10.38, fig.width  = 8.3, purl = FALSE, eval = eval_big_fig}
opar <- par(mfrow = c(4, 2), mar = c(2, 2, 1, 1))
...
```{r prop_patients_with_Kp, fig.height = 10.38, fig.width  = 8.3, purl = FALSE, eval = eval_big_fig}
numerator_df <- ward_dates |> 
  filter(USUBJID %in% Kp_admission) |> 
  group_by(ward) |> 
  group_modify(~ ward_occupancy(.x)) |> 
...
#                   cores = 1) #cores = nb_cores does not work - probbaly beycause of the current R version rejecting the parralel.
```
...
q_array <- boot_msm(Kp_msm_fit, qmatrix.msm, 1000, cores = 1) #cores = nb_cores does not work - probbaly beycause of the current R version rejecting the parralel
```
...
Summary: The "estimate" covariate was not associated with infection risk when modeled as a continuous, dichotomized, or categorized variable. In the quartile-based analysis, no significant differences were observed across groups, and no clear dose-response relationship was evident.
...
```{r}
fit_test <- bsm(state ~ days, USUBJID, Kp_msm_cov, covariates = list("1-2" = ~ DIAB_EOD))
summary(fit_test)
...
vars <- c("estimate", "upper", "lower", "estimate_quartile", "otherCRE", "age",
          "GENDER", "Ab_history", "HealthcareHistory", "CORMOBIDITY", "CPD", "DIAB_EOD",
...
```{r}
univariable_model <- function(v, data) {
  fit <- try(bsm(state ~ days, USUBJID, data,
...
The function that makes the univariable models table:

```{r}
make_uni_table <- function(df, vars) {
  vars |> 
    set_names() |>
    map_dfr(univariable_model, data = df, .id = "variable")
}
```

The function that makes the nN table:

```{r}
make_nN_table <- function(df, vars) {
  df |> 
    filter(days == 0) |> 
    select(!! vars) |>
    get_nN()
...
Kp_uni_table <- make_uni_table(Kp_msm_cov, vars)
Kp_nN_table <- make_nN_table(Kp_msm_cov, vars)
Kp_uni_table
Kp_nN_table
```
...
```{r}
covariates1 = list("1-2" = ~ estimate + age + SEV_RENAL + problem_alcohol + problem_drug + early_DOT + otherCRE)
covariates2 = list("1-2" = ~ estimate_quartile + age + SEV_RENAL + problem_alcohol + problem_drug + early_DOT + otherCRE)
covariates3 = list("1-2" = ~ age + SEV_RENAL + problem_alcohol + problem_drug + early_DOT + otherCRE)

covariates4 = list("1-2" = ~ estimate + age + SEV_RENAL + CPD + DIAB_EOD + problem_alcohol + problem_drug  + early_DOT + wardtype + hospitaltype + ICDGroup + otherCRE)

(Kp_msm_cov_fit <- bsm(state ~ days, USUBJID, Kp_msm_cov, covariates = covariates4))
(Kp_msm_cov_fit1 <- bsm(state ~ days, USUBJID, Kp_msm_cov, covariates = covariates1))
(Kp_msm_cov_fit2 <- bsm(state ~ days, USUBJID, Kp_msm_cov, covariates = covariates2))
(Kp_msm_cov_fit3 <- bsm(state ~ days, USUBJID, Kp_msm_cov, covariates = covariates3))
```
...
Ec_uni_table <- make_uni_table(Ec_msm_cov, vars)
Ec_nN_table <- make_nN_table(Ec_msm_cov, vars)
Ec_uni_table
Ec_nN_table
```
...
```{r}
covariates_Ec = list("1-2" = ~ estimate + age + SEV_RENAL + CPD + DIAB_EOD+ problem_alcohol + problem_drug  + early_DOT + wardtype + hospitaltype + ICDGroup + otherCRE) 

covariates_Ec1 = list("1-2" = ~ estimate + age + early_DOT + wardtype + ICDGroup + otherCRE) 
